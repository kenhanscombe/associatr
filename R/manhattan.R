...
#' @param trait_label A character vector to be used as label. Default =
#' "phenotype".
#' @param trim Observations with p-values larger than the trim value
...
#' @param chr_color A character vector of 2 colours to be used on
...
#'
#' @details SNP id (\code{snp}), Allele 1 (\code{a1}), and regression
...
#' @importFrom magrittr "%>%"
#' @export
manhattan <- function(data, trait_label = "phenotype", trim = 0.001,
    ...
    chr_midpoints <- tapply(df$POS, df[[chromosome]],
    ...
    df %>%
        ...
        annotate("text", x = max_pos, y = -log10(5e-08), vjust = 1,
                ...
                size = 2.5, col = "orange") +
        geom_point(aes(color = factor(df[[chromosome]])), na.rm = TRUE,
                       size = 1, shape = 19, ...) +
        ...
        theme(
            ...
            axis.text.x = element_text(color = chr_color[1], size = 6,
                                       vjust = 2.5),
